...
async function getAdInfoFromAdGroup(adgroupId) {
  const url = `${TIKTOK_BASE_URL}/open_api/v1.3/ad/get/?advertiser_id=${ADVERTISER_ID}&page_size=1&page=1&filtering=${encodeURIComponent(
    JSON.stringify({ adgroup_ids: [adgroupId] })
  )}`;
...
async function getTiktokOneVideoInfo(material_id, tiktokClient) {
  try {
    const path = "/open_api/v1.3//file/video/ad/search/";
    const payload = {
      advertiser_id: ADVERTISER_ID,
      filtering: JSON.stringify({ material_ids: [material_id] }),
    };
...
async function prepareCreative(ad, videoId, imageId, adInfo) {
  ...
  if (ad.adgroup_name.toLowerCase().includes(" || android || ")) {
    creative.landing_page_url =
      "https://play.google.com/store/apps/details?id=enterprises.dating.boo";
  } else if (ad.adgroup_name.toLowerCase().includes(" || ios || ")) {
    if (ad.adgroup_name.toLowerCase().includes(" || general dating || ")) {
      creative.landing_page_url =
        "https://apps.apple.com/us/app/boo-personality-dating-app/id1498407272";
    } else if (ad.adgroup_name.toLowerCase().includes(" || gaming || ")) {
      creative.cpp_url =
        "https://apps.apple.com/us/app/boo-dating-friends-chat/id1498407272?ppid=54875da2-aa8c-4453-b8ae-7c77bc313c0e";
    } else if (ad.adgroup_name.toLowerCase().includes(" || anime || ")) {
      creative.cpp_url =
        "https://apps.apple.com/us/app/boo-dating-friends-chat/id1498407272?ppid=47b17bcd-18a8-4061-8cc1-f516a37ee245";
    }
  }
...
async function getOriginalFilenameFromUrl(url) {
  if (DRYRUN !== "NO")
    return { buffer: {}, originalFilename: "DRYRUN_FILENAME" };
...
async function sendBatch(group, creativeBatch, tiktokClient) {
  ...
  try {
    ...
    if (resp.data?.ad_ids?.length > 0) {
      console.log(
        `✅ Created ad on ad group ${group.adgroup_name} - ${group.adgroup_id}: ${resp.data.ad_ids}`
      );
      return {
        file_processed: "SUCCESS",
      ...
      console.log(
        `❌ failed created ad on group ${group.adgroup_name} - ${group.adgroup_id}: ${resp.message}`
      );
      return {
        file_processed: "SUCCESS",
        create_ad_status: "FAILED",
